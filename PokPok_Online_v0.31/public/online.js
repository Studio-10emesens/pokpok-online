...
const SESSION_KEY='pokpok_session_v031',ROOM_KEY='pokpok_room_v031',ROOM_TOKEN_KEY='pokpok_room_token_v031';
let ws=null,snapshot=null,prevSnapshot=null,account=null,selectedCard=null,selectedTier='debutant',lastEventId=null,rulePage=1,musicOn=true,fxOn=true,musicMode='menu',authMode='login',reconnectAttempted=false,eventBusy=false,avatarContext='profile',wheelRotation=0,audioCtx=null;
...
function syncMusic(){const menu=$('menuMusic'),game=$('gameMusic');if(!menu||!game)return;if(!musicOn){menu.pause();game.pause();return}const play=musicMode==='menu'?menu:game,stop=musicMode==='menu'?game:menu;stop.pause();play.volume=musicMode==='menu'?.10:.28;if(play.paused)play.play().catch(()=>{})}
...
function connect(){
  ...
  ws.onopen=()=>{let s=localStorage.getItem(SESSION_KEY)||localStorage.getItem('pokpok_session_v030')||localStorage.getItem('pokpok_session_v029');if(s){localStorage.setItem(SESSION_KEY,s);send({type:'resume_account',sessionToken:s})}};
...
function handle(m){
  ...
  if(m.type==='account'){
    ...
    const c=localStorage.getItem(ROOM_KEY)||localStorage.getItem('pokpok_room_v030')||localStorage.getItem('pokpok_room_v029'),t=localStorage.getItem(ROOM_TOKEN_KEY)||localStorage.getItem('pokpok_room_token_v030')||localStorage.getItem('pokpok_room_token_v029');
  ...
  if(m.type==='joined'){localStorage.setItem(ROOM_KEY,m.code);localStorage.setItem(ROOM_TOKEN_KEY,m.token);$('roomCodeLabel').textContent=m.code;$('queueBox').classList.add('hidden');$('roomBox').classList.remove('hidden');return}
...
function clearRoomLocal(){[ROOM_KEY,ROOM_TOKEN_KEY,'pokpok_room_v030','pokpok_room_token_v030','pokpok_room_v029','pokpok_room_token_v029'].forEach(k=>localStorage.removeItem(k))}

/* ---------- Écrans / compte ---------- */
function switchScreen(game){$('lobby').classList.toggle('active',!game);$('game').classList.toggle('active',game);$('quitBtn').classList.toggle('hidden',!game);$('roundLabel').textContent=game&&snapshot?`Manche ${snapshot.round} • V0.31 ONLINE`:'V0.31 ONLINE';setMusicMode(game?'game':'menu')}
...
function showHome(){$('authCard').classList.add('hidden');$('homeCard').classList.remove('hidden');$('walletTop').classList.remove('hidden');switchScreen(false)}
function updateAccountUI(){
  ...
  $('dailyBtn').disabled=!account.dailyAvailable;$('dailyBtn').textContent=account.dailyAvailable?'🎁 +50 rubis du jour':'✓ Rubis du jour récupérés';
...
$('dailyBtn').onclick=()=>{fx('bell');send({type:'claim_daily'})};
...
$('quickBtn').onclick=()=>{fx('click');send({type:'quick_match',...matchMeta()})};$('createBtn').onclick=()=>{fx('click');send({type:'create_room',...matchMeta()})};$('joinBtn').onclick=()=>{fx('click');send({type:'join_room',code:$('roomCodeInput').value.toUpperCase()})};$('cancelQueueBtn').onclick=()=>send({type:'cancel_queue'});$('startOnlineBtn').onclick=()=>{fx('bell');send({type:'start_game'})};
...
function render(){
  ...
  const myTurn=snapshot.currentIndex===snapshot.meIndex;$('humanSeat').classList.toggle('active-turn',myTurn);$('humanSeat').classList.toggle('active',myTurn);syncHumanTurnBadge31(myTurn);renderOpponents();renderHand(me);renderCenter();renderActions(me);renderTargets();renderResult();updateClockDom();requestAnimationFrame(()=>{fitHumanHand31();alignHumanActions31()});
}
...
function renderHand(me){
  const h=$('humanHand'),cards=me.hand||[];if(selectedCard&&!cards.some(c=>c.id===selectedCard))selectedCard=null;h.innerHTML='';h.dataset.count=String(cards.length);const selectable=selectablePhase();
  cards.sort((a,b)=>{if(a.type!==b.type)return a.type==='normal'?-1:1;if(a.type==='bonus')return ({blocage:'Blocage',bombe:'Bombe',chance:'Chance',pecheur:'Pêcheur',voleur:'Voleur'}[a.bonus]||'').localeCompare(({blocage:'Blocage',bombe:'Bombe',chance:'Chance',pecheur:'Pêcheur',voleur:'Voleur'}[b.bonus]||''),'fr');return (a.value||0)-(b.value||0)||({C:0,D:1,H:2,S:3}[a.suit]||0)-({C:0,D:1,H:2,S:3}[b.suit]||0)});cards.forEach((c,i)=>{const b=document.createElement('button');b.className='hand-card';if(selectable)b.classList.add('selectable');if(selectedCard===c.id)b.classList.add('selected');b.style.setProperty('--fan-rot',`${(i-(cards.length-1)/2)*2.15}deg`);const img=document.createElement('img');img.src=imgFor(c);img.alt=c.type==='bonus'?c.bonus:c.id;b.appendChild(img);b.onclick=()=>{if(!selectable)return;fx('click');selectedCard=selectedCard===c.id?null:c.id;renderHand(me);renderActions(me)};h.appendChild(b)});$('handEval').textContent=me.eval?`${me.eval.name}${me.eval.points?` • ${me.eval.points} pts`:''}`:'-';requestAnimationFrame(()=>{fitHumanHand31();alignHumanActions31()})
}
...
/* ---------- Ajustements V0.31 : V0.27 sur PC + main mobile toujours entière ---------- */
function fitHumanHand31(){
  const hand=$('humanHand');if(!hand)return;const cards=[...hand.querySelectorAll('.hand-card')];const n=cards.length;if(!n)return;
  const mobile=innerWidth<=700||innerHeight<=560;if(!mobile){cards.forEach(c=>{c.style.removeProperty('height');c.style.removeProperty('margin-left')});return}
  const portrait=innerHeight>=innerWidth;const avail=Math.max(230,hand.clientWidth-12);const ratio=416/603;
  let h=portrait?Math.min(138,Math.max(112,innerHeight*.176)):Math.min(118,Math.max(82,innerHeight*.29));
  let w=h*ratio;let step=n===1?w:(avail-w)/(n-1);step=Math.max(w*.31,Math.min(w*.88,step));let overlap=step-w;
...
function alignHumanActions31(){
  ...
  const sr=seat.getBoundingClientRect(),rects=cards.map(c=>c.getBoundingClientRect()),cardsBottom=Math.max(...rects.map(r=>r.bottom)),scoreTop=score.getBoundingClientRect().top,ah=Math.max(28,actions.getBoundingClientRect().height||28),free=Math.max(0,scoreTop-cardsBottom-ah);let center=(cardsBottom+free/2-6+ah/2)-sr.top;center=Math.max(ah/2+3,Math.min(sr.height-ah/2-3,center));actions.style.setProperty('--action-center-y31',`${center.toFixed(1)}px`)
}
window.addEventListener('resize',()=>requestAnimationFrame(()=>{fitHumanHand31();alignHumanActions31()}));
...
function renderResult(){
  const m=$('roundModal'),r=snapshot.roundResult;if(!r){m.classList.add('hidden');return}m.classList.remove('hidden');$('resultTitle').textContent=r.title;$('resultNote').textContent=r.note;$('rubiesResult').textContent=r.rubiesNote||'';updateResultCountdown();const wrap=$('resultHands');wrap.innerHTML='';
...
async function animateVoleur(ev){const a=centerOf(seatEl(ev.actor)),t=centerOf(seatEl(ev.target));showBurst(ev,1250);if(ev.transfer==='take'){await handMove(a,t,260,0,'🫴','VOLE');await Promise.all([flyCard('assets/card_back.jpg',t,a,340,0,-10),handMove(t,a,330,0,'🤚','RÉCUPÈRE')])}else if(ev.transfer==='give'){await Promise.all([flyCard('assets/card_back.jpg',a,t,350,0,10),handMove(a,t,340,0,'🤚','REND')])}spawnSparkles(ev.transfer==='take'?a:t,22,'gold')}
...
async function playEvent(ev,prev,next){
  if(eventBusy&&ev.kind!=='reaction')await wait(80);eventBusy=true;try{
    ...
    if(['chance','blocage','voleur','pecheur','bombe'].includes(ev.kind)){
      ...
      showBurst(ev,ev.kind==='bombe'?1900:1550);eventAccent31(ev.kind==='bombe'?'bomb':ev.kind,ev.kind==='bombe'?18:11);if(ev.kind==='blocage'&&ev.target!==undefined){comicPoof31(ev.target,'BLOQUÉ !','🚫',520,true);comicPoof31(ev.target,'BONK !','✨',900,false)}if(ev.kind==='chance'){comicPoof31(ev.actor,'CHANCE !','🍀',430,true)}if(ev.kind==='pecheur'){comicPoof31(ev.actor,'PÊCHE !','🐟',420,true)}if(ev.kind==='bombe'&&ev.target!==undefined){comicPoof31(ev.target,'BOUM !','💥',500,true)}
...
/* ---------- Règles ---------- */
$('rulesBtn').onclick=()=>{$('rulesModal').classList.remove('hidden');renderRule();fx('click')};$('closeRules').onclick=()=>$('rulesModal').classList.add('hidden');function renderRule(){$('rulePage').src=`assets/rules/page-${String(rulePage).padStart(2,'0')}.jpg`;$('ruleCounter').textContent=`${rulePage} / 16`;$('rulesPageCounter').textContent=`Page ${rulePage} / 16`}function prevRule(){rulePage=rulePage<=1?16:rulePage-1;renderRule();fx('click')}function nextRule(){rulePage=rulePage>=16?1:rulePage+1;renderRule();fx('click')}$('prevRule').onclick=prevRule;$('nextRule').onclick=nextRule;$('prevRuleText').onclick=prevRule;$('nextRuleText').onclick=nextRule;

/* ---------- Démarrage ---------- */
$('playersChoiceText').textContent='2 joueurs';$('targetChoiceText').textContent='Partie rapide - 500 points';setAuthMode('login');renderAvatarGrid();connect();syncMusic();
