...
function connect(){
  ...
  ws.onopen=()=>{let s=localStorage.getItem(SESSION_KEY)||localStorage.getItem('pokpok_session_v033')||localStorage.getItem('pokpok_session_v032')||localStorage.getItem('pokpok_session_v031')||localStorage.getItem('pokpok_session_v030')||localStorage.getItem('pokpok_session_v029');if(s){localStorage.setItem(SESSION_KEY,s);send({type:'resume_account',sessionToken:s})}};
...
function handle(m){
  ...
  if(m.type==='account'){
    ...
    const c=localStorage.getItem(ROOM_KEY)||localStorage.getItem('pokpok_room_v033')||localStorage.getItem('pokpok_room_v032')||localStorage.getItem('pokpok_room_v031')||localStorage.getItem('pokpok_room_v030')||localStorage.getItem('pokpok_room_v029'),t=localStorage.getItem(ROOM_TOKEN_KEY)||localStorage.getItem('pokpok_room_token_v033')||localStorage.getItem('pokpok_room_token_v032')||localStorage.getItem('pokpok_room_token_v031')||localStorage.getItem('pokpok_room_token_v030')||localStorage.getItem('pokpok_room_token_v029');
  ...
  if(m.type==='left_room'){clearRoomLocal();snapshot=null;prevSnapshot=null;selectedCard=null;selectedCards.clear();$('quitModal').classList.add('hidden');$('roundModal').classList.add('hidden');showHome();toast('Vous avez quitté la table.');return}
  if(m.type==='snapshot'){
...
function clearRoomLocal(){[ROOM_KEY,ROOM_TOKEN_KEY,'pokpok_room_v033','pokpok_room_token_v033','pokpok_room_v032','pokpok_room_token_v032','pokpok_room_v031','pokpok_room_token_v031','pokpok_room_v030','pokpok_room_token_v030','pokpok_room_v029','pokpok_room_token_v029'].forEach(k=>localStorage.removeItem(k))}

/* ---------- Écrans / compte ---------- */
function switchScreen(game){$('lobby').classList.toggle('active',!game);$('game').classList.toggle('active',game);$('quitBtn').classList.toggle('hidden',!game);$('roundLabel').textContent=game&&snapshot?`Manche ${snapshot.round} • V0.34 ONLINE`:'V0.34 ONLINE';setMusicMode(game?'game':'menu')}
...
function renderOpponents(){
  ...
  idxs.forEach((idx,k)=>{const p=snapshot.players[idx],s=document.createElement('section');s.id=`seat-player-${idx}`;s.className=`opponent seat-player ${pos[k]||'seat-top'}`;if(snapshot.currentIndex===idx)s.classList.add('active-turn','active','thinking');if(snapshot.validTargets?.includes(idx))s.classList.add('targetable');
    ...
    if(snapshot.validTargets?.includes(idx))s.onclick=e=>{if(!e.target.closest('button')){fx('click');send({type:'bonus_target',target:idx})}};
    const h=s.querySelector('.mini-hand');for(let c=0;c<p.handCount;c++){const img=document.createElement('img');img.src='assets/card_back.jpg';img.className='mini-card';img.style.setProperty('--mini',c);img.alt='Carte cachée';h.appendChild(img)}wrap.appendChild(s)
  });
}
...
function renderHand(me){
  ...
  const pecheurMode=snapshot.phase==='pecheur_discard'&&snapshot.currentIndex===snapshot.meIndex;
  if(selectedCard&&!cards.some(c=>c.id===selectedCard))selectedCard=null;
  for(const id of [...selectedCards])if(!cards.some(c=>c.id===id))selectedCards.delete(id);
  if(!pecheurMode)selectedCards.clear();
  h.dataset.count=String(cards.length);const selectable=selectablePhase();
  ...
  const existing=new Map([...h.querySelectorAll('.hand-card')].map(el=>[el.dataset.cardId,el]));
  cards.forEach((c,i)=>{
    let b=existing.get(c.id);if(b)existing.delete(c.id);
    if(!b){b=document.createElement('button');b.className='hand-card';b.type='button';b.dataset.cardId=c.id;const img=document.createElement('img');img.src=imgFor(c);img.alt=c.type==='bonus'?c.bonus:c.id;b.appendChild(img)}
    b.classList.toggle('selectable',selectable);b.classList.toggle('inactive',!selectable);
    const selected=pecheurMode?selectedCards.has(c.id):selectedCard===c.id;b.classList.toggle('selected',selected);
    b.style.setProperty('--fan-rot',`${(i-(cards.length-1)/2)*2.15}deg`);
    const img=b.querySelector('img'),src=imgFor(c);if(img.getAttribute('src')!==src)img.src=src;img.alt=c.type==='bonus'?c.bonus:c.id;
    b.onclick=e=>{
      if(!selectable)return;e.preventDefault();e.stopPropagation();fx('click');
      if(pecheurMode){
        const need=Math.max(1,Number(snapshot.pecheurRemaining)||3);
        if(selectedCards.has(c.id))selectedCards.delete(c.id);else if(selectedCards.size<need)selectedCards.add(c.id);
        [...h.querySelectorAll('.hand-card')].forEach(el=>el.classList.toggle('selected',selectedCards.has(el.dataset.cardId)));
      }else{
      ...
      renderActions(me);requestAnimationFrame(()=>{alignHumanActions32();protectTextLayout32()});
    };
    h.appendChild(b);
  });
...
function renderActions(me){
  ...
  else if(ph==='pecheur_discard'){const need=Math.max(1,Number(snapshot.pecheurRemaining)||3),hint=document.createElement('span');hint.className='action-hint';hint.textContent=`Pêcheur : sélectionnez ${need} cartes (${selectedCards.size}/${need})`;a.appendChild(hint);const b=actionButton(`Défausser les ${need} cartes`,'primary primary-action',()=>{if(selectedCards.size===need){fx('click');send({type:'pecheur_discard_many',cardIds:[...selectedCards]})}});b.disabled=selectedCards.size!==need;a.appendChild(b)}
  else if(['normal_discard','chance_discard','final_discard'].includes(ph)){const b=actionButton('Défausser la carte','primary primary-action',()=>{if(selectedCard){fx('click');send({type:'discard',cardId:selectedCard})}});b.disabled=!selectedCard;a.appendChild(b)}
...
function renderTargets(){
  const chooser=$('targetChooser'),self=$('humanSeat'),isSelf=snapshot.validTargets?.includes(snapshot.meIndex);
  self.classList.toggle('targetable',!!isSelf);self.onclick=isSelf?e=>{if(e.target.closest('.hand-card,.actions,.reaction-wrap'))return;fx('click');send({type:'bonus_target',target:snapshot.meIndex})}:null;
  if(snapshot.phase==='voleur_pick'&&snapshot.currentIndex===snapshot.meIndex&&snapshot.pendingVoleur){
    const target=snapshot.players[snapshot.pendingVoleur.target],count=Math.max(0,target?.handCount||0);
    chooser.classList.remove('hidden');chooser.innerHTML=`<div class="target-title">🦝 VOLEUR — Choisissez une carte chez ${escapeHtml(target?.name||'ce joueur')}</div><div class="target-sub">Les cartes restent cachées. Choisissez-en une comme dans la V0.27.</div><div class="steal-card-row">${Array.from({length:count},(_,i)=>`<button class="steal-card" style="--steal-i:${i}" data-steal-index="${i}" aria-label="Prendre la carte ${i+1}"><img src="assets/card_back.jpg" alt="Carte cachée"><span>${i+1}</span></button>`).join('')}</div><div class="raccoon-helper" aria-hidden="true"><span>🦝</span><b>🐾</b></div>`;
    chooser.querySelectorAll('.steal-card').forEach(b=>b.onclick=()=>{fx('click');send({type:'voleur_pick',index:Number(b.dataset.stealIndex)})});return;
  }
  chooser.classList.add('hidden');chooser.innerHTML='';
}


/* ---------- Ajustements V0.34 : aucune collision texte / cartes en portrait ou paysage ---------- */
...
function returnToMenu(){clearRoomLocal();snapshot=null;prevSnapshot=null;selectedCard=null;selectedCards.clear();$('roundModal').classList.add('hidden');$('roomBox').classList.add('hidden');showHome()}
...
async function playEvent(ev,prev,next){
  if(eventBusy&&ev.kind!=='reaction')await wait(80);eventBusy=true;try{
    ...
    if(['chance','blocage','voleur','pecheur','bombe'].includes(ev.kind)){
      fx('bonus');
      if(ev.kind==='pecheur'&&ev.discardSteps){const seat=centerOf(seatEl(ev.actor)),pile=centerOf($('discardPile'));for(let i=0;i<ev.discardSteps;i++){flyCard('assets/card_back.jpg',{x:seat.x+(i-(ev.discardSteps-1)/2)*16,y:seat.y},pile,280,i*70,(i-1)*6);handMove(seat,pile,270,i*70,'🤚','DÉFAUSSE')}await wait(300+ev.discardSteps*70);comicPoof31(ev.actor,`${ev.discardSteps}/3`,'🐟',90,false);return}
