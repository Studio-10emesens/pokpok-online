...
function onConnection(ws){
  send(ws,{type:'hello',version:'0.34',tiers:TIER_STAKES,signupRubies:SIGNUP_RUBIES,wheelAverage:50});
  ws.on('message',async raw=>{let m;try{m=JSON.parse(raw)}catch{return}try{
    ...
    if(m.type==='pecheur_discard_many'){
      if(room.currentIndex!==i||room.phase!=='pecheur_discard')return;
      const need=Math.max(1,Number(room.pecheurRemaining)||3),ids=Array.isArray(m.cardIds)?[...new Set(m.cardIds.map(String))]:[];
      if(ids.length!==need)return send(ws,{type:'error',message:`Pêcheur : sélectionnez exactement ${need} cartes.`});
      const valid=ids.every(cid=>p.hand.some(c=>c.id===cid));if(!valid)return send(ws,{type:'error',message:'Une carte sélectionnée n’est plus disponible.'});
      for(const cid of ids){const c=room.discard(i,cid,false);if(!c)return}
      room.pecheurRemaining=0;room.phase='post_discard';room.broadcastEvent('pecheur',`${p.name} défausse ${need} cartes face cachée avec Pêcheur.`,{actor:i,discardSteps:need,multiDiscard:true});return;
    }
    if(m.type==='discard'){
...
server.listen(PORT,()=>console.log(`POK POK Online v0.34 sur http://localhost:${PORT}`));
