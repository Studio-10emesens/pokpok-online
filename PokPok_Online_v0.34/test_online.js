...
const os=require('os');
const dataDir=fs.mkdtempSync(path.join(os.tmpdir(),'pokpok-v034-'));
...
(async()=>{const sockets=[];try{
  await wait(350);const a=await open(),b=await open(),c=await open(),d=await open();sockets.push(a,b,c,d);await Promise.all([reg(a,'Alice34'),reg(b,'Bob34'),reg(c,'Chloe34'),reg(d,'Dany34')]);
  ...
  const six=[d];for(let n=0;n<5;n++){const w=await open();sockets.push(w);await reg(w,`Six34_${n}`);six.push(w)}
  ...
  console.log("OK V0.34 : 50 rubis à l'inscription, roulette 10-100 (moyenne 50), mises, matchmaking 3/6, pot, confidentialité, chrono 45 s et pioche synchronisée.");
  process.exitCode=0;
