...
const SESSION_KEY='pokpok_session_v032',ROOM_KEY='pokpok_room_v032',ROOM_TOKEN_KEY='pokpok_room_token_v032';
...
/* ---------- Écrans / compte ---------- */
function switchScreen(game){$('lobby').classList.toggle('active',!game);$('game').classList.toggle('active',game);$('quitBtn').classList.toggle('hidden',!game);$('roundLabel').textContent=game&&snapshot?`Manche ${snapshot.round} • V0.32 ONLINE`:'V0.32 ONLINE';setMusicMode(game?'game':'menu')}
...
/* ---------- Ajustements V0.32 : aucune collision texte / cartes en portrait ou paysage ---------- */
...
function protectTextLayout32(){
  ...
  const table=$('centerStatus'),seat=$('humanSeat');if(table&&seat&&(innerWidth<=760||innerHeight<=600)){const tr=table.getBoundingClientRect(),sr=seat.getBoundingClientRect();if(tr.bottom>sr.top-8){const delta=tr.bottom-(sr.top-8);const zone=document.querySelector('.online-game-screen .v05-table-zone');if(zone)zone.style.setProperty('--v032-lift',`${Math.ceil(delta)}px`)}}
}
