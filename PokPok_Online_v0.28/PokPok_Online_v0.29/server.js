...
function userPublic(u){return u?{username:u.username,displayName:u.displayName,rubies:u.rubies,avatar:u.avatar,dailyAvailable:u.lastDaily!==todayKey(),dailyAmount:DAILY_RUBIES}:null}
...
function registerUser(username,secret,avatar){
  ...
  users[key]={id:id(8),username:key,displayName:cleanName(username),salt,hash:passwordHash(secret,salt),rubies:DAILY_RUBIES,lastDaily:todayKey(),createdAt:now(),avatar:cleanAvatar(avatar)};
...
class Room{
  ...
  startRound(){
    clearTimeout(this.roundTimer);this.round++;this.drawPile=shuffle(buildDeck());this.discardPile=[];this.reshuffles=0;this.pokAnnouncer=null;this.finalQueue=[];this.finalCurrent=null;this.pendingVoleur=null;this.pecheurRemaining=0;this.roundResult=null;this.players.forEach(p=>{p.hand=[];p.blocked=false});
    ...
    this.currentIndex=this.dealerIndex;this.phase='normal_draw';this.lastEvent={id:id(4),kind:'deal',text:`Manche ${this.round} : distribution carte par carte.`,ts:now(),dealOrder};this.broadcast();
  }
  ensureDraw(){
  ...
  advanceTurn(idx){
    ...
    if(p.blocked){p.blocked=false;this.currentIndex=idx;this.phase='blocked_pause';this.broadcastEvent('blocked',`${p.name} passe son tour.`,{target:idx});setTimeout(()=>{if(this.phase==='blocked_pause'&&this.currentIndex===idx)this.advanceTurn(this.nextIndex(idx))},850);return}
    this.currentIndex=idx;this.phase='normal_draw';this.broadcast();
  }
  ...
  announcePok(i){
    ...
    this.pokAnnouncer=i;this.players.forEach(x=>x.blocked=false);this.finalQueue=[];let j=this.nextIndex(i);while(j!==i){this.finalQueue.push(j);j=this.nextIndex(j)}this.phase='pok_transition';this.broadcastEvent('pokpok',`${p.name} annonce POK POK ! Tous les adversaires jouent un dernier tour.`,{actor:i});setTimeout(()=>this.beginNextFinal(),420);return true;
  }
  beginNextFinal(){if(this.pokAnnouncer===null)return;if(!this.finalQueue.length){this.showdown();return}this.finalCurrent=this.finalQueue.shift();this.currentIndex=this.finalCurrent;this.phase='final_choice';this.broadcast()}
  ...
  showdown(){
    ...
    let rubiesNote='';if(gameOver)rubiesNote=this.settleRubies(winners);this.roundResult={title,note,rubiesNote,gameOver,reveal:this.revealHands(),deltas,winners,pot:this.pot};this.phase=gameOver?'game_over':'round_over';this.broadcast();if(!gameOver)this.scheduleNextRound();
  }
  endNull(reason){this.roundResult={title:'Manche nulle',note:reason,rubiesNote:'',gameOver:false,reveal:[],deltas:{},winners:[],pot:this.pot};this.phase='round_over';this.broadcast();this.scheduleNextRound()}
  scheduleNextRound(){clearTimeout(this.roundTimer);this.roundTimer=setTimeout(()=>{if(this.phase==='round_over'){this.dealerIndex=this.nextIndex(this.dealerIndex);this.startRound()}},45000)}
  nextRound(requester){if(this.phase!=='round_over'||requester!==this.hostIndex())return false;clearTimeout(this.roundTimer);this.dealerIndex=this.nextIndex(this.dealerIndex);this.startRound();return true}
}
...
function onConnection(ws){
  send(ws,{type:'hello',version:'0.29',tiers:TIER_STAKES,daily:DAILY_RUBIES});
  ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return}try{
    ...
    if(m.type==='claim_daily'){const u=requireUser(ws);if(!u)return send(ws,{type:'error',message:'Connectez-vous.'});if(u.lastDaily===todayKey())return send(ws,{type:'error',message:'Les 50 rubis du jour ont déjà été récupérés.'});u.lastDaily=todayKey();u.rubies+=DAILY_RUBIES;saveUsers();sendAccount(ws);return}
    ...
    if(m.type==='create_room'){
      const u=requireUser(ws);if(!u)return send(ws,{type:'error',message:'Connectez-vous d’abord.'});const opts=roomOpts(m,false),stake=TIER_STAKES[opts.tier];if(u.rubies<stake)return send(ws,{type:'error',message:`Il faut ${stake} rubis pour créer ce salon.`});removeFromQueues(ws);const code=roomCode(),room=new Room(code,opts);rooms.set(code,room);const idx=room.addPlayer(ws.userKey,ws,id(),0);attach(ws,room,idx);return;
    }
    if(m.type==='join_room'){
      const u=requireUser(ws);if(!u)return send(ws,{type:'error',message:'Connectez-vous d’abord.'});const room=findRoom(m.code);if(!room)return send(ws,{type:'error',message:'Salon introuvable.'});if(room.started)return send(ws,{type:'error',message:'La partie a déjà commencé.'});if(room.full())return send(ws,{type:'error',message:'Salon complet.'});if(u.rubies<room.stake)return send(ws,{type:'error',message:`Il faut ${room.stake} rubis pour rejoindre ce salon.`});const idx=room.addPlayer(ws.userKey,ws,id());if(idx===-2)return send(ws,{type:'error',message:'Votre compte est déjà dans ce salon.'});attach(ws,room,idx);return;
    }
    if(m.type==='reconnect'){
    ...
    if(m.type==='start_game'){if(i!==room.hostIndex())return;const r=room.start();if(!r.ok)send(ws,{type:'error',message:r.message});return}
    ...
    if(m.type==='discard'){
      ...
      if(room.phase==='chance_discard'){const c=room.discard(i,String(m.cardId),false);if(!c)return;room.postDiscard(i);return}
...
setInterval(()=>{const cutoff=now()-ROOM_TTL_MS;for(const[code,r]of rooms)if(r.updatedAt<cutoff&&r.players.every(p=>!p?.connected)){clearTimeout(r.roundTimer);rooms.delete(code)}},60000).unref();
server.listen(PORT,()=>console.log(`POK POK Online v0.29 sur http://localhost:${PORT}`));
