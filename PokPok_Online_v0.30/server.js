...
const WHEEL_REWARDS=[5,10,20,30,50,100];
const WHEEL_WEIGHTS=[28,25,20,15,9,3];
...
function userPublic(u){return u?{username:u.username,displayName:u.displayName,rubies:u.rubies,avatar:u.avatar,dailyAvailable:u.lastDaily!==todayKey(),dailyAmount:DAILY_RUBIES,wheelAvailable:u.lastWheel!==todayKey()}:null}
...
function registerUser(username,secret,avatar){
  ...
  users[key]={id:id(8),username:key,displayName:cleanName(username),salt,hash:passwordHash(secret,salt),rubies:DAILY_RUBIES,lastDaily:todayKey(),lastWheel:'',createdAt:now(),avatar:cleanAvatar(avatar)};
...
function onConnection(ws){
  send(ws,{type:'hello',version:'0.30',tiers:TIER_STAKES,daily:DAILY_RUBIES});
...
server.listen(PORT,()=>console.log(`POK POK Online v0.30 sur http://localhost:${PORT}`));
