...
const SESSION_KEY='pokpok_session_v030',ROOM_KEY='pokpok_room_v030',ROOM_TOKEN_KEY='pokpok_room_token_v030';
...
function connect(){
  ...
  ws.onopen=()=>{let s=localStorage.getItem(SESSION_KEY)||localStorage.getItem('pokpok_session_v029');if(s){localStorage.setItem(SESSION_KEY,s);send({type:'resume_account',sessionToken:s})}};
...
function handle(m){
  ...
  if(m.type==='account'){
    ...
    const c=localStorage.getItem(ROOM_KEY)||localStorage.getItem('pokpok_room_v029'),t=localStorage.getItem(ROOM_TOKEN_KEY)||localStorage.getItem('pokpok_room_token_v029');
...
function clearRoomLocal(){[ROOM_KEY,ROOM_TOKEN_KEY,'pokpok_room_v029','pokpok_room_token_v029'].forEach(k=>localStorage.removeItem(k))}

/* ---------- Écrans / compte ---------- */
function switchScreen(game){$('lobby').classList.toggle('active',!game);$('game').classList.toggle('active',game);$('quitBtn').classList.toggle('hidden',!game);$('roundLabel').textContent=game&&snapshot?`Manche ${snapshot.round} • V0.30 ONLINE`:'V0.30 ONLINE';setMusicMode(game?'game':'menu')}
...
function render(){
  ...
  $('humanSeat').classList.toggle('active-turn',snapshot.currentIndex===snapshot.meIndex);renderOpponents();renderHand(me);renderCenter();renderActions(me);renderTargets();renderResult();updateClockDom();
}
...
function renderOpponents(){
  ...
  idxs.forEach((idx,k)=>{const p=snapshot.players[idx],s=document.createElement('section');s.id=`seat-player-${idx}`;s.className=`opponent seat-player ${pos[k]||'seat-top'}`;if(snapshot.currentIndex===idx)s.classList.add('active-turn');if(snapshot.validTargets?.includes(idx))s.classList.add('targetable');
...
function renderActions(me){
  ...
  else if(ph==='voleur_give'){const b=actionButton('Rendre cette carte','primary primary-action',()=>{if(selectedCard){fx('click');send({type:'voleur_give',cardId:selectedCard})}});b.disabled=!selectedCard;a.appendChild(b)}
  ...
  else if(ph==='final_choice'){a.appendChild(actionButton('Conserver','secondary',()=>send({type:'final_keep'})));a.appendChild(actionButton('Piocher puis défausser','primary primary-action',()=>send({type:'final_draw'})))}
}
...
function handMove(from,to,duration=300,delay=0,emoji='🤚',label=''){return new Promise(resolve=>setTimeout(()=>{const h=document.createElement('div');h.className='fx-hand';h.textContent=emoji;h.dataset.label=label;h.style.left=`${from.x-31}px`;h.style.top=`${from.y-31}px`;$('fxLayer').appendChild(h);requestAnimationFrame(()=>h.style.transform=`translate(${to.x-from.x}px,${to.y-from.y}px) rotate(-13deg) scale(1.08)`);setTimeout(()=>{h.style.opacity='0';setTimeout(()=>{h.remove();resolve()},100)},duration)},delay))}
...
async function playEvent(ev,prev,next){
  if(eventBusy&&ev.kind!=='reaction')await wait(80);eventBusy=true;try{
    ...
    if(ev.kind==='pokpok'){fx('pok');showBurst(ev,2600);const p=centerOf(seatEl(ev.actor));spawnSparkles(p,62,'pok');setTimeout(()=>spawnSparkles(p,36,'pok'),350);return}
    ...
    if(['chance','blocage','voleur','pecheur','bombe'].includes(ev.kind)){
      ...
      showBurst(ev,ev.kind==='bombe'?1800:1450);
...
/* ---------- Démarrage ---------- */
setAuthMode('login');renderAvatarGrid();connect();syncMusic();
