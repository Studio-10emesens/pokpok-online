...
const os=require('os');
const dataDir=fs.mkdtempSync(path.join(os.tmpdir(),'pokpok-v030-'));
...
(async()=>{const sockets=[];try{
  await wait(350);const a=await open(),b=await open(),c=await open(),d=await open();sockets.push(a,b,c,d);await Promise.all([reg(a,'Alice30'),reg(b,'Bob30'),reg(c,'Chloe30'),reg(d,'Dany30')]);
  ...
  d.send(JSON.stringify({type:'spin_wheel'}));const wheel=await next(d,m=>m.type==='wheel_result');if(![5,10,20,30,50,100].includes(wheel.reward))throw new Error('Récompense roulette invalide');d.send(JSON.stringify({type:'spin_wheel'}));const wheelAgain=await next(d,m=>m.type==='error');if(!/déjà|jour/i.test(wheelAgain.message))throw new Error('La roulette doit être limitée à une fois par jour');
  ...
  const six=[d];for(let n=0;n<5;n++){const w=await open();sockets.push(w);await reg(w,`Six30_${n}`);six.push(w)}
  ...
  console.log('OK V0.30 : comptes, 50 rubis/jour, roulette quotidienne, contrôle des mises, matchmaking 3 et 6 joueurs, pot, confidentialité, chrono 45 s et pioche synchronisée.');
  process.exitCode=0;
